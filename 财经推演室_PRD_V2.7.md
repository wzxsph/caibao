...
| 对象                 | 精确引用                                                                                                   |
| -------------------- | ---------------------------------------------------------------------------------------------------------- |
| 应用仓 `master`      | `9b5bd02503c951a8b416e66bdd81f48ba89931d5`                                                                 |
| 应用 PR              | [展示 #3](https://github.com/wzxsph/douyin/pull/3)、[同域媒体 #4](https://github.com/wzxsph/douyin/pull/4) |
| Pages 工作流         | <https://github.com/wzxsph/douyin/actions/runs/29970251130>，成功                                          |
...
| 测试                 | client 45、server 131、Playwright 8，均通过                                                                |
...
**[产品决策]** `media-import/authorized-douyin/download-manifest.json` 是完整输入的唯一事实源；当前公开推荐集合必须严格等于其中仍有效且由应用仓 `src/showcase/public-video-ids.json` 显式指定的 10 条。不得从 `posts6.json`、`videos.md`、旧视频 URL、随机评论或底座 fixture 补位。
...
图 1：入口关系参考。该图是历史代码渲染目标图；V2.7 的实际工程页面与文案以线上原型和 `9b5bd025` 为准。
...
flowchart TD
  ...
  Release --> Stage["Actions 校验公开 10 条"]
  Stage --> Media
```
...
- `appCommit=9b5bd02503c951a8b416e66bdd81f48ba89931d5`
...
- 线上浏览器：10 个原作链接、作者页 5/5；首条 Pages 同域 MP4 `readyState=4` 且持续播放，完整请求 200、Range 206、`Content-Type: video/mp4`。点击暂停/关闭恢复由既有 Playwright 用例锁定。
...
- Pages 源：`wzxsph/douyin@9b5bd025`。
