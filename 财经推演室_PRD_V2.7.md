...
| 当前公开形态 | 竖屏推荐流 + 作者作品页；从 25 条清单中显式选择 10 条展示，两位作者各 5 条      |
| 入口         | 视频关键点出现财包轻触点；不是外链、不是页面跳转                                |
| 播放         | 入口曝光不停播；点击进入暂停；完成、跳过或关闭后恢复进入前状态                  |
| 声音         | 首次静音自动播放并显式邀请开启声音；用户手势内解除静音并播放，失败可重试        |
...
1. 完整清单、内容生成与媒体准备保留 25 条；公开推荐只展示其中显式配置的 10 条，而不是固定 4 条或全量 25 条。
2. 普通推荐只读取清单派生目录，不出现底座原视频或旧 mock。
3. 入口曝光继续播放，用户主动点开财包后自动暂停。
4. 退出财包时恢复进入前播放状态，不 seek、不改变音量/静音/倍速。
5. 自动触点不设固定数量上限；旧“最多 4 个”限制取消。
6. 原型可通过 GitHub Pages 公开展示；完整派生媒体保存在 GitHub Release，Actions 校验后把公开 10 条暂存进 Pages 同域 artifact，并逐条标注作者和原作品。
7. 当前学习内容必须明确标注 LLM Mock、未用最终 ASR/OCR、未经财经审核、非投资建议。
8. 前端、Express 后端、生成管线与测试统一迁入 `caibao/apps/web`，后续只在主仓维护；首次有声播放采用浏览器兼容的显式声音入口，不允许永久强制静音或静默播放失败。
...
| Pages 媒体           | 10 个 MP4 + 10 个 JPG，同域暂存共 68,687,281 bytes                                                         |
| 触点                 | 141 个，全部为 `automatic`；当前每视频 3–6 个                                                              |
| 测试                 | 精简主仓 client 42、server 131、Playwright 9，均通过；含显式有声入口测试                                   |
...
- 从 25 条清单中选择 10 条的公开推荐流，两位作者各 5 条。
- 两个作者页，各自只展示清单内作品。
- 六类观看中轻交互。
- 时间轴重访入口。
- 进入暂停、退出恢复的播放状态机。
...
```mermaid
flowchart TD
  Feed["推荐流 · 10 条清单子集"] --> Author["作者页 · 各 5 条"]
  Feed --> Invite["关键点轻触点"]
...
**[产品决策]** `apps/web/media-import/authorized-douyin/download-manifest.json` 是完整输入的唯一事实源；当前公开推荐集合必须严格等于其中仍有效且由 `apps/web/src/showcase/public-video-ids.json` 显式指定的 10 条。不得从 `posts6.json`、`videos.md`、旧视频 URL、随机评论或底座 fixture 补位。
...
- 完整清单有 25 个不同视频 ID，小Lin说 15 条、大陆姓陆 10 条。
- 公开配置有 10 个不同且可在完整清单中解析的 ID，两位作者各 5 条。
- 完整生成 bundle 为 25 条保存标题、作者、作者 slug、原作品 URL、观察到的发布时间、AI 生成披露、时长、尺寸和指纹；当前线上运行时只投影公开 10 条。
...
- 部署不含媒体的 Pages artifact，并让推荐页停止提供媒体；
- 不得只依靠前端隐藏，因为已有 Release 和 Pages 媒体直链仍可能访问；
- 保留最小审计记录，不继续分发媒体副本。
...
- 页面提供 1/10 等当前位置提示及上一条/下一条无障碍按钮。

![历史目标图：视频流与财包轻提示；V2.7 实际页面以线上原型为准](assets/prd-v2/01-feed-nudge.png)

图 1：入口关系参考。该图是历史代码渲染目标图；V2.7 的实际工程页面与文案以主仓 `apps/web` 和主 Pages 为准。
...
- 财包 pause/release 不修改 `muted`、`volume`、`playbackRate`；用户显式声音操作可以修改 `muted`。
- 恢复不得调用会顺带把音量设为 1 的通用播放器方法。
- 播放器拒绝 `play()` 时保留暂停状态，不伪造已恢复事件。

## 5.4 首次声音与自动播放

**[现状事实]** 浏览器通常会拒绝没有用户手势的有声自动播放。V2.7 不把这一平台限制伪装成“视频没有声音”，也不承诺首帧自动有声。

**[产品决策]** 播放器采用以下可恢复策略：

1. 首次访问先静音尝试自动播放，同时显示至少 44px 高的“点击开启声音”入口；不得只在右侧放一个难发现的小静音按钮。
2. 用户点击声音入口、视频或中央播放键时，必须在同一次用户手势中执行解除静音和 `play()`。
3. 成功后在当前站点保存声音偏好；后续视频优先按该偏好尝试有声播放。
4. 若浏览器拒绝 `play()`，保留可操作的“点击有声播放”入口，不显示虚假的播放状态。
5. 用户显式静音时更新偏好；财包邀请、进入、作答与退出不得擅自改变用户声音选择。
...
```mermaid
flowchart TD
  Repo["wzxsph/caibao · apps/web"] --> Actions["主仓 GitHub Actions"]
  Actions --> Pages["caibao GitHub Pages · Vue/Vite"]
  User["移动/桌面浏览器"] --> Pages
  Pages --> Bundle["公开 10 条 Catalog + Experience"]
  Pages --> Media["Pages 同域 · 10 MP4 + 10 JPG"]
  Pages --> Douyin["抖音原作品链接"]
  Builder["本地 manifest + FFmpeg + Mock generator · 完整 25 条"] --> Full["完整 showcase-bundle.json"]
  Public["public-video-ids.json · 10 条"] --> Bundle
  Full --> Bundle
  Builder --> Release["GitHub Release · 25 MP4 + 25 JPG"]
  Release --> Stage["主仓 Actions 校验公开 10 条"]
  Stage --> Media
```
...
- `appCommit=caibao/main` 的具体发布提交 SHA
...
| ---- | -------------------------------- | --------------------- | --------------------------- | ----------------------------------------------- |
| F-01 | 推荐集合只来自有效清单与公开配置 | Feed/Catalog          | `feed_impression`           | 完整源 25；公开集合精确 10，失败为空态          |
| F-02 | 公开 10 条均有真实归属与原作链接 | Feed/Author           | `source_link_click`         | 当前线上 10/10 链接                             |
...
| F-11 | 作者页为公开清单子集             | AuthorPage            | page view                   | 5/5，与 Feed 交集一致                           |
| F-12 | Mock 边界清晰                    | Feed/Sheet            | impression                  | 显示未用最终 ASR/OCR、未审核、非投资建议        |
| F-13 | 报告只用真实事件                 | Report                | `report_view`               | 无事件不伪造掌握证据，降级仍非空                |
| F-14 | 到期与撤权可停止分发             | Release/Pages/Catalog | retire                      | 前端空态 + Release 下架 + 无媒体 Pages artifact |
| F-15 | 首次声音可发现、可恢复           | Player               | sound enable/retry          | 入口≥44px；点击后有声播放；偏好保存；失败可重试 |
...
- 前端 Vitest：10 个文件、42 个测试（精简主仓快照）。
- 服务端 Vitest：21 个文件、131 个测试。
- 两套 TypeScript type-check。
- Production build。
- Playwright：9 个测试，覆盖 390×844、393×852、430×932、1280×900，并包含首次有声入口。
- `pnpm audit --prod`：无生产依赖漏洞。
- `git diff --check`。
- 真实媒体目录：25 ready、0 excluded；HEAD 200、Range 206。
- 迁移前线上浏览器：10 个原作链接、作者页 5/5；首条同域 MP4 `readyState=4` 且持续播放，完整请求 200、Range 206、`Content-Type: video/mp4`。声音根因是应用强制静音而非媒体损坏；主仓用例已锁定显式有声入口、unmute + play 与偏好保存。
...
- Release 上传资产数必须为 50；Pages staging 必须精确输出公开 10 个 MP4 + 10 个 JPG，共 68,687,281 bytes，页面不得请求旧媒体或 Release 直链。

## 13.3 播放 / 触点

- 曝光继续播放；点击后暂停；三种退出按前态恢复。
- 首次静音自动播放时显示可见有声入口；一次用户手势后 `muted=false && paused=false`；浏览器拒绝时可重试，刷新/切换按站点偏好恢复。
...
- Pages 源：`wzxsph/caibao/apps/web`；发布提交以对应主仓 workflow 的 `headSha` 为准。
- 媒体：Release `showcase-media-20260723-v1` 保存完整派生；Pages artifact 同域暂存公开 10 条。
- 完整生成 bundle 固化 25 条清单目录和 Mock Experience，运行时由独立配置选择 10 条。
- Git 中不含视频文件。
...
- 媒体撤权/到期：删除或下架 Release，部署不含媒体的 Pages artifact，再发布空态/移除目录版本并验证两类直链。
...
- 25 条清单 → 25 条完整内容；公开配置 → 10 条推荐目录与作者页（5/5）。
- 可复现 H.264/AAC 媒体准备与 GitHub Release。
- 公开媒体经 Actions 校验后以 Pages 同域 `video/mp4` 提供，修复 Release 直链播放失败。
- 六类确定性 Mock 生成与 Schema 校验。
- 轻触点、暂停/恢复、半屏和时间轴重访。
- Pages 部署、来源归属、披露和四视口 E2E。
- 取消自动触点“最多 4 个”限制。
- 显式修复首次声音入口和自动播放拒绝恢复；财包状态机仍不改变声音属性。
- 将精简前后端、生成管线和测试迁入 `caibao/apps/web`，主仓成为唯一代码源。
...
1. 推荐从固定四条改为完整处理 25 条，随后按用户最新决定公开其中 10 条。
2. 从本地-only 改为用户要求的公开 GitHub Pages 工程展示；完整媒体放独立 Release，公开 10 条暂存进 Pages 同域 artifact。
...
Given 首次访问且浏览器不允许自动有声，When 视频静音自动播放，Then 显示至少 44px 的“点击开启声音”。  
Given 用户点击有声入口，When 浏览器接受该手势，Then 视频同时解除静音并继续播放，且当前站点记住偏好。  
Given 浏览器拒绝播放，When 请求失败，Then 显示“点击有声播放”供重试，不伪造正在播放。
...
Given 清单有效 25 条且公开配置有效 10 条，When 打开推荐流，Then 页面集合和 10 个原作链接与公开配置一致，作者为 5/5。  
Given 清单为空或校验失败，When 打开推荐流，Then 显示空态且不请求旧 URL。

## B.4 到期

Given 权利窗口未续期且到达 2026-08-22 截止，When 执行 retire，Then 推荐停止，Release 与 Pages 媒体资产均不可再通过直链获得。
