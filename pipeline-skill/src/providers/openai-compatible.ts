/**
 * OpenAI-compatible chat-completions client (works for MiniMax, Ark, OpenAI, …).
 *
 * Extracted verbatim from `apps/web/server/src/providers/openai-compatible.ts`.
 * The pipeline uses it for both the semantic-graph extraction stage and the
 * per-cue payload authoring stage.
 */
      ...
      try {
        ...
        if (!parsed.success) {
          throw new AppError(
            ...
            { status: 502, details: { issueCount: parsed.error.issues.length } }
          )
        }
        return parsed.data
      } catch (error) {
