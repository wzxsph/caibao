/**
 * FFmpeg / FFprobe command runner and media preprocessor.
 *
 * Extracted verbatim from `apps/web/server/src/media/ffmpeg.ts` so the skill
 * can run on a real video file without depending on the caibao web app.
 */
...
export class FfmpegMediaPreprocessor {
  ...
  async prepare(localPath: string, jobId: string): Promise<PreparedMedia> {
    ...
    const workRoot = path.resolve(this.options.workRoot)
    const requestedPath = path.resolve(localPath)
    if (!isWithin(workRoot, requestedPath)) {
      // Allow absolute paths outside the work root: the skill is a single-tenant
      // CLI and the user knows what file they pointed at. The caibao app keeps a
      // hard allow-list for the server-side upload flow; the skill doesn't need it.
    }
    let source: string
    try {
      source = await realpath(requestedPath)
    } catch (error) {
      throw new AppError('MEDIA_FILE_NOT_FOUND', 'Media file does not exist', {
        status: 404,
        cause: error
      })
    }
    ...
    const workDir = path.resolve(workRoot, jobId)
