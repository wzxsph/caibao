/**
 * Domain contracts for the extracted finance-video analysis pipeline.
 *
 * These are the same shapes used inside the caibao web app
 * (`apps/web/server/src/domain/contracts.ts`). Keeping them verbatim lets the
 * pipeline output be consumed by the same downstream renderer without a
 * translation step.
 */
import { z } from 'zod'

// ── Evidence (consumed by ASR / OCR providers) ──────────────────────────────
...
// ── Trigger candidates / direction resolutions ─────────────────────────────
...
import type { CueKind } from './payload-contracts.js'
...
// ── Prepared media (output of the parse stage) ──────────────────────────────
