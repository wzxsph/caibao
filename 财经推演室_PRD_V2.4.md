...
| 应用仓 `refer/douyin` | Vue/Vite/Express 唯一实现 | 文档事实以提交 `17f06c66` 为基线；并发未提交代码不纳入事实声明 | 不向公共 upstream push，不覆盖他人工作树 |
...
[现状事实] 已记录的离线回归基线为 20 个前端 Vitest、84 个服务端 Vitest和 6 个 Playwright；类型检查、构建和生产依赖审计通过。该事实只证明 fixture/fake-client 下的契约和编排，不证明真实 Provider、真实内容、授权或发布质量。

[现状事实] 当前运行 Demo 仍只绑定三类触点，使用工程占位媒体；财经条目曾出现黑屏并复用错误作者元数据。六类渲染单测通过不等于六类真实时间轴 E2E 已通过。
...
[现状事实] 核心有界生成管线、方向规则和 CoverageReport 已由 fake-client 测试证明；真实授权媒体、真实 Provider 质量/延迟/成本、持久任务和人工发布链仍未验证。
...
| Review/Publish Gate | ReviewManifest 校验、批准动作与不可变发布指针 |
...
| `POST /content/versions/:contentVersion/approve` | 批准已 reviewed 版本 | 权利/证据/机制/安全/测试全绿；与生成动作分离 |
| `POST /content/versions/:contentVersion/publish` | 切换发布指针 | 仅接受 approved；幂等、可回滚 |
| `POST /content/versions/:contentVersion/retire` | 停止新会话 | 权利到期/下架/安全事件留审计记录 |

[产品决策] 若实现团队选择 CLI + 受控配置代替 HTTP，必须满足相同 Schema、权限、幂等、审计和自动化测试；不得通过手改运行 fixture 模拟“已发布”。
...
| F-18 | ReviewManifest 完整才可批准 | Review Gate | review/approve log | T-REVIEW-01 |
...
2. 再写 ReviewManifest、approve/publish 分离、发布指针回滚测试。
...
| T-REVIEW-01 | 缺任一必审维度或真实审核人 | approve 返回 `REVIEW_INCOMPLETE` |
| T-PUB-01 | 客户端请求 draft/reviewed | 返回 `EXPERIENCE_NOT_APPROVED` |
| T-PUB-02 | 发布、回滚与 retire | 指针只指不可变 approved 版本，审计完整 |
...
- ReviewManifest 完整，approve 与 publish 是独立、有权限、可审计动作。
...
4. 先写失败测试，再实现 PM adapter、版本向量与 Review/Publish Gate。
...
| PM 内容被直接发布 | adapter 保持 draft；approve/publish 负向测试 |
...
- 新增 ReviewManifest、最小受控 approve/publish/retire 链和发布指针回滚。
