...
日期：2026-07-23  
语言：简体中文  
状态：Historical Review Candidate（已被 PRD V2.6 取代；只读保留，不再接受签字）  
当前权威：`财经推演室_PRD_V2.0.md`；其中“不自动暂停”条款已被用户 2026-07-23 最新指令覆盖  
...
> 历史提示：用户 2026-07-23 后续裁决将入口收敛为“财包 POI 微入口”，并要求普通首页、财经 Demo、长视频推荐只读取授权 manifest 的有效子集。最新候选见 `财经推演室_PRD_V2.6.md`；本文件保留 V2.5 决策快照，不再作为最新实现口径。

# 0. 文档控制与一页决策

## 0.1 权威性

[产品决策] 本文是 V2.5 **联合评审候选**，不是已经上线的实现。V2.0 在 V2.5 完成联合评审、同步权威指针并创建批准标签前仍是已批准基线；但用户 2026-07-23 最新指令“进入财包交互时自动暂停”优先于 V2.0、V2.3 和 V2.4 中相反条款，从本候选起作为实现约束。

[产品决策] V2.4 已完成版本治理、内容审核生命周期和文档先行门禁设计，但因播放交互约束发生规范性变化，被 V2.5 取代为历史候选。V2.3 与 V2.4 均只读保留，不回写成已批准版本。
...
| 本 PRD V2.5 | 最新 Review Candidate | 评审通过前否；自动暂停为用户直接裁决 |
...
5. 创建指向批准提交的 annotated tag `prd-v2.5-approved`。

## 0.2 四类事实标签

- [现状事实] 已在固定文件、源码、测试或界面中核验；需写明核验边界。
- [产品决策] V2.5 候选锁定的目标行为；其中“进入财包自动暂停”来自用户直接裁决，其余获批后成为强制实施口径。
...
![图 1｜PM 原型中的进入财包暂停状态取证；V2.5 仅采纳暂停意图，不采纳大遮罩、作者位替换或静态能力结论](assets/prd-v2.3/moneybaby-online-agent.png)

## 0.4 本轮新增裁决

1. 邀请出现时视频继续播放；用户主动进入财包交互后，系统立即暂停在当前时间点。
2. 退出交互时按进入前状态恢复：原本播放才自动续播，原本暂停则继续暂停；禁止跳到节点后或证据窗其他位置。
3. 采用“文档版本先行、内容版本随后”的实施顺序；未批准的 PRD 变化不得静默进入 approved 内容。
4. PRD 版本与内容版本彻底解耦；内容修订通常只升内容版本，不自动升 PRD。
5. 增加最小可审计发布链：ReviewManifest、权利记录、审核结论和批准动作必须分离于模型生成。
6. 内容包必须记录其依据的已批准 PRD 标签和产品仓提交；Review Candidate 只能生产 draft。
7. 把产品评审门与内容发布门拆开：外部视频授权可阻塞内容发布，但不制造 PRD 无法批准的循环等待。
...
| 仓库 | 固定角色 | 当前核验基线 | 边界 |
|---|---|---|---|
| 产品仓 `/caibao` | PRD、PDF、图、研究、ADR、计划与交接 | V2.5 候选基于 `main@dff09ee` | 不把嵌套代码仓当普通目录提交 |
...
- 复述、证据回看和报告的信息架构。
- 用户主动进入交互时暂停、退出后回到视频的基础意图；具体状态恢复必须按 V2.5 规则重写。
...
- 最多 4 次自动邀请、最多 6 个节点、六类可渲染模板和进入后自动暂停的半屏。
...
![图 2｜V2.5 延续的参考架构：模型生成候选，人审发布，客户端只运行审核计划](assets/prd-v2.3/03-v23-architecture.png)
...
| 维度 | 示例 | 变更触发 |
|---|---|---|
| PRD baseline | `prd-v2.5-approved@<product-sha>` | 产品范围、交互、契约或门禁改变 |
...
内容版本不得使用模糊的“V2.5”代替。每个 approved 内容必须固化完整版本向量；任一关键维度变化都产生新内容版本或新 Schema/规则版本，不能覆盖原记录。
...
- 合并后创建 `prd-v2.5-approved`；未满足前不创建标签。
...
1. 联合评审 V2.5；播放行为修改留在独立文档提交，V2.4 只读保留。
2. 获批后原子更新权威指针并打批准标签。
...
- 邀请曝光仍不打断播放；用户主动进入财包后改为自动暂停。
- 完成、跳过和关闭统一按进入前播放状态恢复，且不自动 seek。
- 新增 `wasPlayingBeforeInteraction`、`pausePositionMs` 与幂等播放状态转换验收。
- 保持 ≤48vh、无蒙层、自动 ≤4/节点 ≤6/间隔 ≥45 秒、六类交互、版本治理和审核发布门禁不变。

## 18.2 保持不变的产品红线

- 轻邀请不暂停；进入交互自动暂停且可从原位置恢复。财包不替作者、无全屏蒙层、播放中不实时生成。
...
本文件没有任何已签字结论。真实评审人、结论、日期、阻塞和修改请求记录在 `docs/reviews/PRD_V2.5_REVIEW.md`。在该记录完成前：

- V2.0 仍为已批准基线，但其中“不自动暂停”条款被用户最新指令覆盖。
- V2.5 仅是 Review Candidate，其余未评审变化不得越权。
- 不创建 `prd-v2.5-approved` 标签。
- 不把 V2.5 PDF、分支或本地提交称为团队已批准基线。

# 附录 A. 评审与实现索引

- 版本治理：`docs/VERSION_GOVERNANCE.md`
- V2.5 评审包：`docs/reviews/PRD_V2.5_REVIEW.md`
- V2.4→V2.5 差异：`docs/PRD_V2.4_TO_V2.5_DIFF.md`
