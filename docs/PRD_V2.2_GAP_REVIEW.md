...
当前后继候选：`../财经推演室_PRD_V2.6.md`（V2.3/V2.4/V2.5 已成为历史候选；V2.0 仍是已批准基线）

> V2.6 后续裁决：关键点入口改为财包 POI 微入口，普通首页/财经 Demo/长推荐只读授权 manifest；本文件的旧媒体与入口结论仅作历史取证。

> 本文件保留 V2.2→V2.3 取证语境；文中的“V2.3 裁决”是历史候选形成依据，不能单独解释为
> 已批准产品决定。V2.2 的“邀请时继续播放、点击进入后暂停”基础意图已由 V2.5 重新采纳；
> 本文对自动暂停的否定属于 V2.3/V2.4 历史口径。当前状态以 V2.5 §0 和 Agent 交接为准。
...
- 当时的线上静态前端位于旧 `douyin` Pages；该入口现已退役，当前入口见主仓 README。
...
- 六个节点全部保留为内容时间轴能力。
- 历史候选曾限定自动邀请最多 4 次，并选择 00:35、01:28、02:32、03:58；V2.6 已取消该独立数量上限，仅保留节点总数 ≤6、自动间隔 ≥45 秒和同时 1 个。
...
5. 自动邀请的 45 秒间隔、同时 1 个和六节点不截断策略通过真机干扰测试；不再验证“最多 4 次”。
