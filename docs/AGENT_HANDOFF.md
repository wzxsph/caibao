...
| 当前文档任务 | 联合评审 V2.4；评审通过前不得切换权威、创建 approved tag 或让内容版本进入 approved |
| 代码第一任务 | 修复 `package.json` 裸 `vite --host` 导致的 `0.0.0.0` 监听，并验证默认脚本只绑定回环地址；以代码仓最新状态复核是否已由并行工作完成 |
...
- 内容时间轴节点最多 6 个；V2.4 候选目标自动邀请最多 4 次、最小间隔 45 秒、同时最多 1 个；忽略只记“未观察”。当前契约仍允许 6 个自动点，是待改差异。
...
- 自动邀请上限仍是 6，尚未按 V2.4 候选收紧为 4；PM 六个候选间隔中 27 秒和 36 秒两处违反 45 秒约束。
...
4. 建 ReviewManifest、approve/publish/retire 分离动作和 ApprovedExperience-only 读取接口。
...
7. 前端仓储改为 ApprovedExperience API-first + 明确 Demo fallback；自动邀请上限改为 4。
