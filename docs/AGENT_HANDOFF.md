...
| 项目 | 当前事实 |
|---|---|
| 产品口径 | V2.0 仍是已批准基线，但“不自动暂停”已被用户最新指令覆盖；`财经推演室_PRD_V2.5.md` 是最新 Review Candidate；V2.3/V2.4 为历史候选 |
| 产品仓 | `/home/samsong/Desktop/maybe/caibao`；主分支 `main`，V2.5 候选起始基线 `dff09ee`，当前提交以 `git rev-parse HEAD` 为准 |
| 代码仓 | `/home/samsong/Desktop/maybe/caibao/refer/douyin`；稳定引用 `feat/caibao-analysis-pipeline@b8ced09d` 与未 push foundation `4b34da1f`；共享工作树由外部 Agent 使用，实时状态必须重查 |
...
| 里程碑 | M0–M2 已完成；M3 已有静态前端切片与有界多阶段管线，Planner 已 4/45、六类 payload 可离线成稿；PM 媒体迁移正有并发工作，M4 授权/供应商/审核发布未完成 |
| 自动化基线 | `b8ced09d`：20 前端 + 102 服务端 + 6 Playwright 全绿；`4b34da1f` 后本轮复核服务端 105/105；完整六类真实时间轴 E2E 仍未验证 |
...
| 当前第一任务 | 代码侧先用 TDD 实现 V2.5 播放状态机；文档侧联合评审 V2.5。评审通过前不得创建 approved tag 或让内容版本进入 approved |
| foundation 安全项 | 未 push 的 `4b34da1f` 已把 `dev/start/serve` 显式绑定 `127.0.0.1` 并加 3 个断言；合并或推送须由后续代码任务单独处理 |
...
3. 具体任务需要时再读：
   - `财经推演室_PRD_V2.5.md`（最新 Review Candidate）
   - `docs/PRD_V2.4_TO_V2.5_DIFF.md`
   - `docs/VERSION_GOVERNANCE.md` 与 `docs/reviews/PRD_V2.5_REVIEW.md`
...
权威优先级：用户最新指令 > 已通过的 PRD（当前为 V2.0）> 本交接 > Architecture/TDD/ADR > 待评审 V2.5 > 历史候选 V2.3/V2.4 > PM V2.2 > 历史材料。播放行为的最新直接裁决是“进入财包自动暂停”，不得被 V2.0/V2.3/V2.4 的旧条款覆盖。
...
本轮还观察到其他 Agent/真实运行任务生成的未跟踪 `output/real-runs/`；其归属不在文档分支，
不要删除、移动或提交，所有文档提交使用显式 pathspec。不要擅自提交或删除上述内容。
...
- 稳定 checkpoint：`feat/caibao-analysis-pipeline@b8ced09d`（Planner 4/45 与六类成稿）。
- 后继本地分支：`refactor/moneybaby-v2.4-foundation@4b34da1f`，未 push，增加回环绑定修复。
...
- 项目 `origin`：`https://github.com/wzxsph/douyin.git`；`feat/caibao-analysis-pipeline@b8ced09d`
  相对远端同名分支领先 5 个提交；当前 refactor 分支尚无上游跟踪。未经授权不 push。

当前观察到外部 Agent 未提交工作树：删除 `public/demo/finance-media-placeholder.webm`，修改
`src/features/finance-cues/fixtures/finance-fed-v1.ts` 与 `src/mock/index.ts`，并新增未跟踪
`public/demo/finance-real-venezuela.mp4`。这些不是本次 PRD 工作的改动，不得 stage、覆盖、回退、
移动或替对方提交；接手时以 `git status --short` 重新确认所有者和最新状态。
...
- 内容时间轴节点最多 6 个；`b8ced09d` 已把自动邀请默认值/硬上限收紧为 4、最小间隔 45 秒、
  同时最多 1 个；忽略只记“未观察”。节点总数 6 与自动邀请 4 必须继续分开建模。
...
- 首包实际主题是美元外溢、全球资本和相对利差，不足以自动证明股票/黄金路径；V2.5 延续“由内容包定义学习目标”。
- 线上/源码/测试差异的完整证据见 `docs/PRD_V2.2_GAP_REVIEW.md` 和 `assets/prd-v2.3/`。

## 3.2 V2.5 文档与版本治理

- V2.5 是最新 Review Candidate；V2.0 仍是已批准基线，V2.3/V2.4 只作历史候选。当前没有任何真实联合签字。
- 用户 2026-07-23 的“进入财包自动暂停”直接裁决立即约束实现；这不代表 V2.5 其余内容已批准。
- V2.5 获批必须同一变更完成 Markdown 状态、PDF、评审记录、AGENTS/交接/计划权威指针，
  并在合并到 `main` 后创建 annotated tag `prd-v2.5-approved`；不得提前创建该 tag。
...
- 规则与完整流程见 `docs/VERSION_GOVERNANCE.md`；待签字项见
  `docs/reviews/PRD_V2.5_REVIEW.md`；最新差异见 `docs/PRD_V2.4_TO_V2.5_DIFF.md`。
...
- 当前播放器/半屏仍按 V2.4“不停播”实现，**尚不满足 V2.5**；不要把已有 6 项 Playwright 当作
  自动暂停与恢复的通过证据。下一代码任务须先替换相反断言，再实现状态机。
...
- **前后端契约现支持全 6 类触点**：InteractionRenderer 可渲染，payload-author 可对六类成稿，
  server/client golden 与单元测试覆盖。`finance-fed-6kinds` 仍是**测试专用 fixture，尚未绑定到
  完整运行时 Demo/mock**，Playwright 仍只跑三类 Demo；真实 Provider 质量和六类真媒体 E2E 未验证。
...
- **3 个新触点类型（quick_judgment/counterexample_flip/concept_compare）的完整运行 E2E**：
  前后端契约、payload 成稿和渲染单测已实现，但 `finance-fed-6kinds` fixture 未绑定完整运行 Demo，
  6 个 Playwright 仍只覆盖三类；真媒体、真机布局与无障碍未做完整验证。
- Planner 默认值和硬上限已在 `b8ced09d` 收紧为自动邀请 4、最小间隔 45 秒；PM 六候选本身
  仍有 27 秒和 36 秒两处冲突，需要 adapter/内容编排选择最多 4 个，而不是原样发布。
...
- 未 push 的 foundation 分支 `4b34da1f` 已将 `pnpm dev/start/serve` 显式绑定 `127.0.0.1`，并增加
  3 个静态断言；本轮在该分支曾复核服务端 105/105。该能力尚未合并或推送，不得写成远端已完成，
  也不得在本次文档任务中继续移植。
...
下一次代码任务先按 V2.5 用 TDD 重构播放状态机；文档侧并行完成 V2.5 联合评审：

1. 先确认并发工作 Agent 和媒体删除/新增的所有者，禁止覆盖工作或把未授权媒体纳入提交。
2. 先写 T-PLAY-00..04：邀请不暂停；播放中进入后暂停；退出从原位置恢复；原本暂停则保持暂停；重复事件幂等。
3. 最小实现 `wasPlayingBeforeInteraction`、`pausePositionMs` 和播放转换状态；不自动 seek、静音、改音量或倍速。
4. 再写 `PATCH /analysis/jobs/:jobId/draft`、`POST /analysis/jobs/:jobId/publish`、
   Review Candidate/draft 不可批准及版本向量的失败测试。
5. job publish 只把已审 draft 物化为不可变 ApprovedExperience；
   `/content/versions/:contentVersion/publish|retire` 才切换或撤销运行发布指针。
6. V2.5 未获批前，不执行内容 approved/published 状态切换。

## 9. 文档批准与代码队列

1. 先联合评审 V2.5；评审意见记录在 `docs/reviews/PRD_V2.5_REVIEW.md`。通过前 V2.0 仍是已批准
   基线，但进入自动暂停按用户直接裁决实现；不创建 approved tag，不启动 approved 内容发布。
2. 代码工作树归属清晰后，先完成 T-PLAY-00..04 与最小播放状态机，不重做半屏视觉层。
3. 评审通过后，同一批准提交切换权威指针并创建 `prd-v2.5-approved` annotated tag。
4. 再为 PM 内容适配、独立版本向量、作者/媒体一致性、
...
7. 用户提供 10–30 秒有处理权视频并确认费用后执行最小 dry run；Docker 仍暂停，不因有 FFmpeg 自动恢复。
8. 前端仓储改为 ApprovedExperience API-first + 明确 Demo fallback；Planner 4/45 已完成，
   继续验证 PM 内容适配后不超过该上限。
9. 把六类触点绑定到时长匹配的授权媒体并补 E2E，再补 Session/Event/Report 与片尾深挖。

## 10. 验证基线

最近一次完整离线/fixture 基线（`b8ced09d`，2026-07-23）：

- 前端 Vitest：**20 项**（原 13；新增 InteractionRenderer 6 类渲染 7 项）。
- 服务端 Vitest：**102 项**，包含 Planner 4/45、六类 payload author、富语义、方向规则与
  CoverageReport；`4b34da1f` 新增 3 个回环绑定断言后，本轮实际复核为 **105/105**。
...
- 全部上述离线测试用 faked client/fixture，不调用计费模型、不消耗额度；六类 server payload 可成稿
  不等于真实 Provider 或完整六类真实时间轴 E2E 已验证。
- PRD V2.3 PDF：历史候选为 A4 28 页；原有记录显示曾全量渲染检查，路径为
  `output/pdf/财经推演室_产品功能PRD_V2.3.pdf`。
- PRD V2.5 PDF：由同名 Markdown 生成。按用户 2026-07-23 最新指令，本轮及后续默认只做
