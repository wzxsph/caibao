...
| 项目 | 当前事实 |
|---|---|
| 产品口径 | V2.0 仍是当前裁决；`财经推演室_PRD_V2.4.md` 是加入独立版本、ReviewManifest 和发布门禁后的 Review Candidate；V2.3 为历史候选 |
| 产品仓 | `/home/samsong/Desktop/maybe/caibao`；V2.4 文档工作分支 `docs/prd-v2.4-governance` 基于 `720a5ff`，当前提交以 `git rev-parse HEAD` 为准 |
| 代码仓 | `/home/samsong/Desktop/maybe/caibao/refer/douyin`，当前检出 `feat/caibao-analysis-pipeline@b8ced09d`；未 push foundation 分支 `4b34da1f` 含回环修复但未检出；外部媒体/fixture 工作树不触碰 |
...
| 当前第一任务 | 联合评审 V2.4；评审通过前不得切换权威、创建 approved tag 或让内容版本进入 approved；代码侧只做不越权的 draft/TDD 基础工作 |
| foundation 安全项 | 未 push 的 `4b34da1f` 已把 `dev/start/serve` 显式绑定 `127.0.0.1` 并加 3 个断言，但当前 `b8ced09d` 未包含；是否移植须由代码任务单独处理 |
...
3. 具体任务需要时再读：
   - `财经推演室_PRD_V2.4.md`（Review Candidate）
   - `docs/VERSION_GOVERNANCE.md` 与 `docs/reviews/PRD_V2.4_REVIEW.md`
...
权威优先级：用户最新指令 > 已通过的 PRD（当前为 V2.0）> 本交接 > Architecture/TDD/ADR > 待评审 V2.4 > 历史候选 V2.3 > PM V2.2 > 历史材料。
...
- 当前检出分支：`feat/caibao-analysis-pipeline`
- 当前已提交 HEAD：`b8ced09d`（Planner 4/45 与六类成稿 checkpoint）。另有本地未 push 分支
  `refactor/moneybaby-v2.4-foundation@4b34da1f`，其回环绑定修复当前未检出。
...
- 首包实际主题是美元外溢、全球资本和相对利差，不足以自动证明股票/黄金路径；V2.4 延续“由内容包定义学习目标”。
- 线上/源码/测试差异的完整证据见 `docs/PRD_V2.2_GAP_REVIEW.md` 和 `assets/prd-v2.3/`。

## 3.2 V2.4 文档与版本治理

- V2.4 是 Review Candidate；V2.0 仍权威，V2.3 只作历史候选。当前没有任何真实联合签字。
- V2.4 获批必须同一变更完成 Markdown 状态、PDF、评审记录、AGENTS/交接/计划权威指针，
  并在合并到 `main` 后创建 annotated tag `prd-v2.4-approved`；本分支不得提前创建该 tag。
...
- 规则与完整流程见 `docs/VERSION_GOVERNANCE.md`；待签字项见
  `docs/reviews/PRD_V2.4_REVIEW.md`；差异见 `docs/PRD_V2.3_TO_V2.4_DIFF.md`。
...
- 未 push 的 foundation 分支 `4b34da1f` 已将 `pnpm dev/start/serve` 显式绑定 `127.0.0.1`，并增加
  3 个静态断言；本轮在该分支曾复核服务端 105/105。但当前检出的 `b8ced09d` 不含该修复，
  不得把它写成当前分支已完成，也不得在本次文档任务中移植。

当前检出基线的安全临时启动方式（直到回环修复经独立代码任务移植）：
...
先完成 V2.4 联合评审与权威决策。代码侧如继续并行，只允许不越权的 draft/TDD 基础工作：

1. 先确认并发媒体删除/新增的所有者，禁止把未授权媒体纳入提交或公开路径。
2. 先写 `PATCH /analysis/jobs/:jobId/draft`、`POST /analysis/jobs/:jobId/publish`、
   Review Candidate/draft 不可批准及版本向量的失败测试。
3. job publish 只把已审 draft 物化为不可变 ApprovedExperience；
   `/content/versions/:contentVersion/publish|retire` 才切换或撤销运行发布指针。
4. V2.4 未获批前，不执行内容 approved/published 状态切换。

## 9. 文档批准与代码队列

1. 先联合评审 V2.4；评审意见记录在 `docs/reviews/PRD_V2.4_REVIEW.md`。通过前 V2.0 仍权威，
   不创建 approved tag，不启动 approved 内容实施。
2. 评审通过后，同一批准提交切换权威指针并创建 `prd-v2.4-approved` annotated tag。
3. 代码工作树归属清晰且干净后，先为 PM 内容适配、独立版本向量、作者/媒体一致性、
   “Review Candidate/draft 不可直接 approved”写失败测试。
4. 建 draft PATCH、ReviewManifest、job publish（物化 approved）、content publish/retire（运行指针）
   分离动作和 ApprovedExperience-only 读取接口。
5. 实现默认跳过、默认不联网、单 Provider 最多一次请求的 live preflight/smoke harness；缺 key/媒体时明确拒绝。
6. 用户提供 10–30 秒有处理权视频并确认费用后执行最小 dry run；Docker 仍暂停，不因有 FFmpeg 自动恢复。
7. 前端仓储改为 ApprovedExperience API-first + 明确 Demo fallback；Planner 4/45 已完成，
   继续验证 PM 内容适配后不超过该上限。
8. 把六类触点绑定到时长匹配的授权媒体并补 E2E，再补 Session/Event/Report 与片尾深挖。
...
- PRD V2.4 PDF：由本分支 Markdown 生成。按用户 2026-07-23 最新指令，本轮及后续默认只做
