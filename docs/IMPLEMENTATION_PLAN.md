...
把现有静态财经触点 Demo 升级为可交接的垂直切片：可以合规接入已授权视频，离线执行 ASR、OCR、语义理解和触点候选规划；审核后由视频内轻提示和不超过半屏的容器消费内容包；观看不中断，最终形成无分数的过程学习总结。当前权威仍为 V2.0；产品目标以 Review Candidate `财经推演室_PRD_V2.4.md` 为候选，V2.4 获批前只能开展不进入 approved 基线的探索和 draft 工作。

## 阶段与完成定义

### D0｜V2.4 文档候选

- [x] 基于 V2.3、PM gap、架构、TDD 与交接形成 V2.4 Review Candidate。
- [x] 增加 PRD/内容/Schema/规则/权重/Prompt/应用/媒体的独立版本治理。
- [x] 增加 ReviewManifest、批准/发布分离和 V2.3→V2.4 追溯材料。
...
- [ ] 合并到产品仓 `main` 后创建 annotated tag `prd-v2.4-approved`。
...
- 不在 V2.4 获批前从并发脏工作树建立内容发布分支，不把未批准产品决策静默写进代码。
