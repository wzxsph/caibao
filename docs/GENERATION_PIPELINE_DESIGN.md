...
状态：已采纳（核心管线已实现；真实内容与发布链路未验证）
日期：2026-07-22
唯一产品裁决：`财经推演室_PRD_V2.0.md`
配套决策记录：`docs/ADR/0003-agentic-generation-pipeline.md`
落点代码仓：`refer/douyin`，分支 `feat/caibao-analysis-pipeline`
...
范围外：人工审核台、publish API、持久化、OAuth、运行时编排、学习足迹——分别属 P1 或运行流。
...
| 候选无 payload | `pipeline/payload-author.ts` 已对作者白名单内的 kind 生成前端形状 payload | 当前 server 授权白名单仍只开启 3 类，另3类保持 candidate-only |
| 三层命名不一致 | server/前端/PRD 已统一为 6 个 `CueKind`，前端六类渲染器均有单测 | 运行时 Demo 和 E2E 仍只绑定原三类 fixture |

确定性 Planner（`pipeline/cue-planner.ts`）的职责保持不变：只 accept/reject，执行 max6/min45s/证据存在/禁词/visualLoad/时长内等硬约束；评分、方向和成稿分别由可追溯的独立阶段负责。
...
- **用 `RENDERABLE_KINDS` 集合门控授权**：当前 server 仍只对 `context_card/condition_slider/causal_stitch` 授权 payload；另 3 个虽已有前端渲染器，但在运行时 Demo/E2E/视觉验收完成前保持 candidate-only。
...
| 3 vs 6 kind | 前端六类渲染器已有单测；新 3 类先完成运行时 fixture/E2E/视觉回归，再扩 server `RENDERABLE_KINDS` 白名单 |
...
2. **落点真实性**：文中每个文件路径在 `refer/douyin` 实际存在，并由代码提交 `50b96560` 落地。
3. **不变量回归**：逐条对照 PRD 红线（离线、draft-only、方向归规则、模型不自发布、注入防护、无总分），确认每条都有对应机制（§8）。
4. **ADR 一致**：`0003` 是 `0002` 证据优先门禁的细化，非替代。

实现后已执行 `pnpm test / type-check / type-check:server / build / test:e2e / audit --prod / git diff --check`：20 个前端 Vitest、84 个服务端 Vitest、6 个 Playwright 全部通过，生产依赖无已知漏洞。新增管线阶段全部由离线 fixture/fake client 驱动，**没有调用计费模型或使用真实媒体**。
