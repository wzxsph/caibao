...
确定性 Planner（`pipeline/cue-planner.ts`）的职责保持不变：只 accept/reject。当前已实现
`max6/min45s/证据存在/禁词/visualLoad/时长内` 等硬约束；V2.4 候选要求把“时间轴节点最多 6”与
“自动邀请最多 4”分开建模。评分、方向和成稿分别由可追溯的独立阶段负责。
