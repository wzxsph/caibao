...
最新候选产品口径：`财经推演室_PRD_V2.6.md`  
历史候选：`财经推演室_产品功能PRD_V2.3.md`、`财经推演室_PRD_V2.4.md`、`财经推演室_PRD_V2.5.md`

## 1. 目的

本文件解决五个问题：哪个 PRD 有权裁决、何时升级文档版本、内容版本如何独立演进、批准与发布如何留下证据，以及 Agent 如何在三个仓库间安全交接。

核心原则是“规范先批准，内容后生产；生成、审核、批准、发布彼此分离”。本文件存在不等于 V2.6 已批准，也不授予任何 Agent 创建批准标签或把 draft 升级为 approved 的权限。用户直接裁决的“进入暂停、POI 微入口、manifest-only 推荐、自动无独立数量上限”优先于旧 PRD 的相反条款，但不会自动批准 V2.6 的其他内容。四套估算触点只能进入本地 `internal_poc`，该范围不是 approved/published。
...
5. Review Candidate；当前为 V2.6。
6. 历史候选 V2.3/V2.4/V2.5、PM V2.2、旧 PRD、原型与口头示例。
...
| POI 命名/尺寸、推荐唯一源、媒体 API 与权利范围变化 | Minor | V2.6 |
...
后继 Approved PRD 生效后，旧版只读保留。Review Candidate 也可以在批准前被新候选取代；V2.3、V2.4 和 V2.5 即属于此类历史候选，不能回写成已批准。

## 5. V2.6 批准动作

V2.6 批准必须是一个可审计的原子变更：

- `财经推演室_PRD_V2.6.md` 状态改为 Approved。
- `docs/reviews/PRD_V2.6_REVIEW.md` 记录真实结论。
- `AGENTS.md`、`docs/AGENT_HANDOFF.md`、`docs/IMPLEMENTATION_PLAN.md`、架构/TDD 指针同步。
- PDF 从同一 Markdown 重新生成并完成用户允许的机器校验。
- 合并到 `main` 后创建 annotated tag `prd-v2.6-approved`。

当前分支不得创建该 tag，也不得预填签字。批准提交建议使用：

```text
docs: approve PRD v2.6 and switch authority
```

## 6. 独立版本向量

批准内容必须固定下列字段，不能用一个“V2.6”替代全部版本：

| 字段 | 格式建议 | 所有者 | 目的 |
|---|---|---|---|
| `prdBaseline` | `prd-v2.6-approved@<product-sha>` | 产品仓 | 证明依据的产品规范 |
...
源 manifest schemaVersion 与产品推荐白名单是两个独立版本维度。服务只兼容已明确实现和测试的 v1/v2；schema 升级、条目新增、作者变化或排序变化只能影响解析，不得扩大固定四个财经 videoId/Experience 映射。当前 v2 的其余 21 条必须记录为 `UNMAPPED`，不能成为替补媒体。
...
generated → draft → in_review → reviewed → approved → published → retired
          ...
          ↘ internal_poc（本地显式范围；不可进入 approved/published）
```
...
| `internal_poc` | 用户直接裁决 + 本地配置 | 仅回环/测试展示；不代表人审、批准或公网分发权 |
...
| 交互 | CueKind、选项、反馈、节点总数 ≤6/自动间隔 ≥45 秒/同时 1 个/≤12 秒/≤48vh；自动无独立数量上限 |
...
文档候选从明确产品仓 SHA 开始。本轮用户要求以 `caibao/main` 为主，V2.6 起始基线为：

```text
base: 26cc11ea5c9d75ab1a633072d04f28737d49750d
branch: main
```

候选文档建议保留审计清晰的提交组：

1. `docs: define POI and manifest-only media in PRD v2.6`
2. `docs: align project entrypoint and handoff with v2.6`
3. `docs: generate PRD v2.6 review PDF`

评审后另建批准提交，不修改上述候选提交。未经用户明确授权不 push、不创建远端 PR、不改写历史。

### 9.2 应用代码仓

只有在 V2.6 获批且并发工作树归属清晰、干净后，才从明确 SHA 建正式内容发布分支。用户已直接授权的本地 `internal_poc` 实施可以在候选期进行，但必须与 approved/published 路径硬隔离。例如正式分支：
...
8. `docs: record evidence against prd-v2.6-approved`
...
当前四条媒体仅获内部 PoC 声明，公网分发或 Provider 尚未就绪可以作为内容发布阻塞；只要不改变产品规范，不应造成 PRD 批准的循环等待。

### 10.2 内容发布门

必须同时具备已批准 PRD baseline、覆盖目标环境且未过期的权利、媒体/作者一致、最终 ASR/OCR/时间码、财经人审、完整版本向量、节点总数 ≤6/自动间隔 ≥45 秒/同时 1 个、六类运行/恢复/安全测试、ReviewManifest 和受控批准/发布动作。自动触点不得设置独立数量上限；`internal_poc` 与当前 2026-08-22 到期声明不能满足生产内容发布门。
